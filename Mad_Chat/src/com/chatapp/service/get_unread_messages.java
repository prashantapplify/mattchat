...
public class get_unread_messages extends AsyncTask<String , Integer, Void>{

	String auth1,accesstoken,senderid,responseBody;
	Context ctx;
	...
	public get_unread_messages(Context ctx, String accesstoken,String senderid, SharedPreferences sp, ListView chat_room_list, int w, int h, InputMethodManager imm,String Chat_Type, LinearLayout chat_room_list1) {
	...
	protected Void doInBackground(String... params) {
		...
		try {
			...
			if(Chat_Type.equalsIgnoreCase("0")){
				httppost = new HttpPost(Constant_URL.get_unread_messages);
			}else{
				 httppost = new HttpPost(Constant_URL.get_unread_messages_of_group);
			}
			...
			try {
				...
				if(Chat_Type.equalsIgnoreCase("0")){
					...
					nameValuePairs.add(new BasicNameValuePair("accesstoken",accesstoken ));
					nameValuePairs.add(new BasicNameValuePair("groupid", senderid));
				}
...
//				Chat_Room.Chat_List_arrayList=new ArrayList<Get_Chat_model>();
				...
				for(int i=0;abc>i;i++){
					...
					Log.d("++", object2.getString("chat"));
					Get_Chat_model contact_list_model=new Get_Chat_model();
					contact_list_model.setMsg(object2.getString("chat"));
					contact_list_model.setSend_id(object2.getString("senderid"));
					contact_list_model.setSend_time(object2.getString("timestamp"));
					Chat_Room.Chat_List_arrayList.add(contact_list_model);
					
				}
				Get_Chat_Adapter ChatList_adp=new Get_Chat_Adapter(ctx, Chat_Room.Chat_List_arrayList,w,h,sp,"unread",imm,chat_room_list1);
